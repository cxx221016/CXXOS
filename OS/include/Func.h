...
const std::string consttxtmemtable="..\\file\\Memtable.txt";
const std::string constbinmemtable="..\\file\\Memtable.bin";
...
private:
    ...
    void compact()
    {
        ...
        BinarySerial::BinarytoSerial(filename,ss.str());
    }
public:
    Memtable(const std::string &filename = constbinmemtable,const std::string& base=constbase,const std::string& ext=constext) 
    {
        ...
        std::string serialdata=BinarySerial::ChooseSerial(filename);
    ...
    friend std::ostream& operator<<(std::ostream& os,const Memtable& mt)
    {
        for(auto& [idx,attr]:mt.memtable)
        {
            os<<attr<<std::endl;
        }
        return os;
    ...
    friend std::istream& operator>>(std::istream& is,Memtable& mt)
    {
        std::string line;
