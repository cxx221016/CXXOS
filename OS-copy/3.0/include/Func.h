...
/*
std::ofstream fout("D:\\MY_CODE\\CPP\\OS\\file\\Memtable.txt");

std::string generate()
{
    std::string base="D:\\MY_CODE\\CPP\\OS\\file\\Mem";
    std::string ext=".txt";
...
public:
    Memtable(const std::string &filename = "D:\\MY_CODE\\CPP\\OS\\file\\Memtable.txt") : filename(filename)
    {
        std::ifstream fin;
        fin.open(filename);
        if (!fin.is_open())
