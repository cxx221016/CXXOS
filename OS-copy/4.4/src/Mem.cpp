...
std::pair<std::string,std::string> Mem::splitPath(const std::string& init)
{
    int hasSlash=init.find_last_of('/');
    int haspoint=init.find('.');
    if(hasSlash==std::string::npos)
    {
        if(haspoint!=std::string::npos)
        {
            return std::make_pair(current->path,init);
        }
        else
        {
            return std::make_pair(init,"");
        }
    }
    else
    {
        if(haspoint!=std::string::npos)
        {
            return std::make_pair(init.substr(0,hasSlash),init.substr(hasSlash+1));
        }
        else
        {
            std::string tmp=current->path+'/'+init;
